...
runtime status is tracked in [`../../ROADMAP.md`](../../ROADMAP.md),
[`../roadmaps/jtml-competitive-features-roadmap.md`](../roadmaps/jtml-competitive-features-roadmap.md),
and [`../roadmaps/ai-native-implementation-roadmap.md`](../roadmaps/ai-native-implementation-roadmap.md).
...
2. **One‑file mental model.** Authors write Friendly JTML 2; Studio shows
   the lowered compatibility form only as a diagnostic surface, never as a
   target.
...
| `Counter`, `Form`, `Dashboard` Studio samples mixed compatibility-backend snippets into Friendly examples. | Samples were ported by hand and never linted. | Samples now use `jtml 2` Friendly examples and reserve the lowered compatibility form for compiler artifacts. |
...
### 3.3 Compatibility IR should stay behind HTML

The old Artifacts panel foregrounded the compatibility lowering tab by default.
That contradicted goal #2: Friendly is the public language, while the
Classic-compatible form is a backend artifact for migration, embedding,
and debugging.

**Shipped baseline.**

- Default the Artifacts panel to `HTML` (the actual runtime output).
- Rename the old tab to `Compatibility IR` / `Lowered Compatibility Form`
  so it reads as a backend artifact instead of a second language choice.

**Remaining proposal.**

- Move `Compatibility IR` behind a diagnostics drawer alongside
  `Bindings JSON`, `State JSON`, semantic usage, and token stream.
- Keep the reference text in the Help panel that explains the contract:
  Friendly is canonical; the Classic-compatible form is backend-only.
