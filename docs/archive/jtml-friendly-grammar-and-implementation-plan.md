# Historical: JTML Friendly Syntax Grammar And Implementation Plan

Status: **historical implementation note** — Friendly JTML 2 is the default
authoring syntax. This document records the grammar/lowering path that got the
project here; it is not the current product philosophy. The user-facing
reference is [`../reference/language-reference.md`](../reference/language-reference.md),
and the active architecture direction is
[`../architecture/observable-first-architecture-roadmap.md`](../architecture/observable-first-architecture-roadmap.md).
...
Implementation note: the current implementation still lowers Friendly source
through a compatibility backend before execution. That backend is an
implementation detail for migration, embedding, and current runtime stability;
new docs, examples, Studio flows, and AI-generated code should present Friendly
JTML plus the observable semantic graph as the source of truth.
...
- `../reference/language-reference.md` updated.
