...
and Studio can preview the app while explaining the semantic graph and generated
artifacts.
...
2. AI authoring contract.
   - Keep [`../reference/ai-authoring-contract.md`](../reference/ai-authoring-contract.md) short enough for prompt context.
...
3. Studio compiler transparency.
   - Show Friendly source, generated HTML, semantic diagnostics, runtime
     preview, local snapshots/history, autosaved drafts, and command palette.
   - Keep the lowered compatibility form available for migration/debugging,
     but never as the primary authoring surface for new users.
...
2. Make `jtml explain` describe:
   ...
   - Shipped richer text and JSON output with these categories plus style
     blocks, semantic component definitions/instances, observable usage
     warnings, and linter diagnostics.
3. Add `jtml fix`:
   - Runs check/lint.
...
5. VS Code/LSP diagnostics.
   ...
   - Shipped native LSP foundation: `jtml lsp` speaks JSON-RPC/LSP over stdio
     ...
     whole-document formatting. See [`../tooling/language-server.md`](../tooling/language-server.md).
   ...
   - Shipped catalog-backed keyword completions: LSP, Studio highlighting,
     VS Code grammar coverage, README, and reference docs are guarded by the
     same canonical `jtml keywords --json` language surface instead of
     independent keyword lists drifting apart.
...
1. Stable HTTP contract for `jtml serve`: `GET /` returns the page,
   ...
   Documented under [`../tooling/runtime-http-contract.md`](../tooling/runtime-http-contract.md) so any backend or tool
...
4. Small C ABI so any language with FFI can host the runtime; keep
   ...
   - Shipped first slice: `include/jtml/c_api.h` and `libjtml` expose
     ...
     Documented under [`../tooling/embedding-c-api.md`](../tooling/embedding-c-api.md).
...
paths surfaced through [`../tooling/runtime-http-contract.md`](../tooling/runtime-http-contract.md):
