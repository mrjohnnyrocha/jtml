...
- catalog-backed Friendly JTML completions plus richer snippets for common
  constructs and same-file/imported user-defined symbols through
  `textDocument/completion`. Imported items advertise their origin module in
  `detail` so editors can disambiguate same-named symbols.
- keyword + user-symbol hover documentation through `textDocument/hover`.
  Built-in keywords follow `jtml keywords --json`, so editor help tracks the
  same canonical mini-reference as the CLI, Studio, and docs. Imported symbols
  are annotated with the origin file.
