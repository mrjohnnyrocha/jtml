...
const std::set<std::string>& commonAttributes() {
    static const std::set<std::string> names = {
        ...
        "data-jtml-route-params", "data-jtml-link", "aria-label",
        "method", "action", "target", "rel",
        "width", "height", "min", "max", "step", "pattern", "rows", "cols"
    };
    return names;
}
...
bool isVoidTag(const std::string& tag) {
    ...
    else if (tag == "checkbox") resolved = "input";
    else {
        auto it = elementAliases().find(tag);
...
bool isKnownLineKeyword(const std::string& word) {
    static const std::set<std::string> names = {
        ...
        "continue", "slot", "style", "route", "effect", "store", "redirect"
    };
...
void emitLine(std::ostringstream& out, int level, const std::string& line) {
    ...
    for (size_t i = 0; i < lines.size(); ++i) {
        ...
        if (tokens[0] == "route") {
            if (tokens.size() != 4 || tokens[2] != "as" || !isQuoted(tokens[1]) || !isComponentName(tokens[3])) {
                throw std::runtime_error("Expected 'route \"path\" as Component' at line " + std::to_string(lines[i].number));
            }
        ...
        for (const auto& bodyLine : body) {
            ...
            if (!bodyTokens.empty() && bodyTokens[0] == "slot") {
                for (const auto& slotLine : slotLines) {
                    ...
                    injected.indent = lines[i].indent + bodyLine.indent + slotLine.indent;
...
ElementResult translateElement(const std::vector<std::string>& tokens, int lineNumber) {
    ...
    while (i < tokens.size()) {
        ...
        if (key == "to" && resolvedTag == "a") {
            ...
            attrs.push_back("href=" + friendlyRouteHref(tokens[i++], lineNumber));
        ...
        if (key == "into") {
            ...
            // checkbox uses onChange; text inputs use onInput
            const std::string eventAttr = (friendlyTag == "checkbox") ? "onChange" : "onInput";
...
bool looksLikeFriendlySyntax(const std::string& source) {
    ...
    while (std::getline(in, raw)) {
        ...
        if (head == "let" || head == "get" || head == "const" || head == "when" || head == "page" ||
            ...
            head == "route" || head == "effect" || head == "store") {
            return true;
        }
        return false;
    }
...
std::string friendlyToClassic(const std::string& source) {
    auto lines = collectLines(source);
    std::map<std::string, std::string> storeActions;
    lines = expandStoreLines(lines, storeActions);
    lines = rewriteStoreActionRefs(lines, storeActions);
    lines = expandComponentLines(lines, collectComponentDefs(lines));
    std::ostringstream out;
    ...
    for (size_t index = 0; index < lines.size(); ++index) {
        ...
        if (head == "use") {
            ...
            emitLine(out, level, (insideBlock ? "" : "define ") + typedName.name + " = " + expr + "\\\\");
            ...
            emitLine(out, level, classicHead + typedName.name + " = " + expr + "\\\\");
            ...
            const std::string expr = friendlyExpression(joinTokens(tokens, nameIndex + 1));
            emitLine(out, level, "derive " + typedName.name + " = " + expr + "\\\\");
        } else if (head == "const") {
            if (tokens.size() < 4) {
            ...
            if (!currentActionName.empty()) {
                redirectActions[currentActionName] = tokens[1];
            }
